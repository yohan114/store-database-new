...
import { PURCHASE_SOURCES, SOURCE_HEAD_OFFICE, sourceShort } from '../../lib/constants.js';
...
export default function Tracker() {
  ...
  const [requestSource, setRequestSource] = useState('all');
  ...
  if (requestSource !== 'all') params.set('requestSource', requestSource);
  if (filter) params.set('filter', filter);

  const { data, isLoading, error, isFetching } = useQuery({
    queryKey: ['items', { s: debSearch, category, vehicle, requestSource, filter, sort, order, page }],
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap gap-3">
          ...
          <select className="input max-w-[12rem]" value={requestSource} onChange={(e) => resetTo(setRequestSource)(e.target.value)}>
            <option value="all">All sources</option>
            {PURCHASE_SOURCES.map((sSrc) => (
              <option key={sSrc} value={sSrc}>{sSrc}</option>
            ))}
          </select>
        </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!isLoading && items.length === 0 && (
              <tr><td colSpan={11} className="px-4 py-10 text-center text-slate-400">No matching records.</td></tr>
            )}
            {items.map((it) => {
        ...
        <td className="px-3 py-2">
          {it.requestSource ? (
            <span className={`whitespace-nowrap rounded px-2 py-0.5 text-xs font-medium ${
              it.requestSource === SOURCE_HEAD_OFFICE ? 'bg-indigo-50 text-indigo-700' : 'bg-teal-50 text-teal-700'
            }`}>{sourceShort(it.requestSource)}</span>
          ) : (
